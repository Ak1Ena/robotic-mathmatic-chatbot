...
**Pre-implementation / documentation phase.** No source code exists yet. Full specification lives in `project.md`, `CONCEPT.md`, `requirement.md`, and `roadmap.md`.
...
1. **Backend** (FastAPI + NumPy/SymPy) computes kinematics/physics; an AI agent determines how to visualize the result
...
The frontend must be a **pure renderer** — no physics logic, no AI reasoning. This separation is a hard architectural requirement (`requirement.md §6.2`).
...
These color assignments are part of the instructional design — must be consistent throughout:
...
- **Real-time slider updates**: simulation state must recompute instantly on input change (no perceptible lag).
